...
def spectrum_stats(freqs, amp):
    ...
    return {
        ...
        "high_freq_ratio_35_80": high_freq_ratio(freqs, amp),
    }
...
def main():
    ...
    with out_stats.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["series", "path", "peak_frequency_hz", "high_freq_ratio_35_80"])
        for suffix, label, _, _ in SERIES:
            writer.writerow([
                label,
                spectra[suffix]["path"],
                stats[suffix]["peak_frequency_hz"],
                stats[suffix]["high_freq_ratio_35_80"],
            ])
    ...
    for suffix, label, _, _ in SERIES:
        print(
            ...
            f"35-80Hz ratio={stats[suffix]['high_freq_ratio_35_80']:.4f}"
        )
