...
class ResidualWideLoss(nn.Module):
    ...
    def forward(self, x, residual_pred, wide_target, residual_target):
        ...
        total = loss_wide + 0.2 * loss_residual + self.mean_weight * loss_mean
...
def plot_history(history):
    ...
    keys = ["total", "mae", "freq", "phase", "residual", "mean"]
    epochs = np.asarray(
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.legend()
    fig.tight_layout()
    fig.savefig(LOG_DIR / "bias_corrected_residual_training_curves.png", dpi=300)
    plt.close(fig)
...
def main():
    ...
    criterion = ResidualWideLoss(mean_weight=args.mean_weight)
    ...
    history = {f"{split}_{key}": [] for split in ("train", "val")
               for key in ("total", "mae", "freq", "phase", "residual", "mean")}
    history["epoch"] = []
    history["lr"] = []
    best_val = float("inf")
    no_improve = 0
    ...
    if args.resume:
        ...
        history_path = LOG_DIR / "bias_corrected_residual_training_history.npy"
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if val["total"] < best_val:
            ...
            }, CHECKPOINT_DIR / "best_bias_corrected_residual_model.pth")
        else:
            no_improve += 1
        ...
        torch.save({
            ...
            "base_c": args.base_c,
            "mean_weight": args.mean_weight,
        }, CHECKPOINT_DIR / "last_bias_corrected_residual_model.pth")

    np.save(LOG_DIR / "bias_corrected_residual_training_history.npy", history)
