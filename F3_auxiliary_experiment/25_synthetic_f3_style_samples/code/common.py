...
def targeted_gain_correction(residual_pred, fs=250, band_low=50, band_high=90,
                              target_ratio=0.35, max_gain=2.0):
    """
    Only apply spectral gain to the configured residual high-frequency band.
...
def residual_band_loss(pred_residual, label_residual, dt=DT):
    """
    Constrain amplitude and phase matching within the residual band (40-90 Hz).
    ...
    band_mask = (freqs >= RESIDUAL_BAND[0]) & (freqs <= RESIDUAL_BAND[1])
...
def residual_energy_ratio_loss(pred_residual, label_residual, dt=DT):
    """
    Constrain the 50-90 Hz energy ratio of predicted residual to match the label.
    ...
    """
    def band_energy_ratio(x, freqs, low=RESIDUAL_GAIN_BAND[0], high=RESIDUAL_GAIN_BAND[1]):
