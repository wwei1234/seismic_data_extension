...
class ResidualPatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        r = np.asarray(self.labels[idx], dtype=np.float32)
        if self.augment:
...
def main():
    ...
    parser.add_argument("--resume", default="", help="Resume from a checkpoint and continue to --epochs.")
    ...
    if args.resume:
        ...
        best_path = DATA_DIR / "checkpoints" / "best_model.pth"
        if best_path.exists():
            best_checkpoint = torch.load(best_path, map_location="cpu")
            best_val = float(best_checkpoint.get("val_loss", checkpoint.get("val_loss", best_val)))
        else:
            best_val = float(checkpoint.get("val_loss", best_val))
        history_path = LOG_DIR / "training_history.npy"
        if history_path.exists():
            old_history = np.load(history_path, allow_pickle=True).item()
            for key in history:
                if key in old_history:
                    history[key] = list(old_history[key])
        print(f"Resuming from {args.resume} at epoch {start_epoch}, best_val={best_val:.6f}")
