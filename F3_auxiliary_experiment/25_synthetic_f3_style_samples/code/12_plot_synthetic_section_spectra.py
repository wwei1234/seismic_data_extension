...
def main():
    ...
    wide_labels_path = DATA_DIR / "synthetic_section_wide_labels.npy"
    wide_labels = (
        np.load(wide_labels_path, allow_pickle=True).item()
        if wide_labels_path.exists()
        else {key: inputs[key] + value for key, value in labels.items()}
    )
    ...
    for n_wells in (2, 3, 4):
        ...
        series = {
            ...
            "wide_label": wide_labels[sid],
            "residual_label": labels[sid],
        }
        spectra = {}
        ...
        ax.axvspan(50, 90, color="tab:red", alpha=0.10, label="50-90 Hz")
        ax.set_xlim(0, 120)
    ...
    for ax, n_wells in zip(axes, (2, 3, 4)):
        ...
        ax.axvspan(50, 90, color="tab:red", alpha=0.10)
        ax.set_xlim(0, 120)
        ax.set_ylim(bottom=0)
        ax.set_xlabel("Frequency (Hz)")
    ...
    for row in rows:
        print(
            ...
            f"peak={row['peak_hz']:.2f}Hz, 50-90 ratio={row['ratio_35_80']:.4f}, "
