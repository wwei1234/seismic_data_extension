...
def save_stats(path, rows):
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["series", "peak_frequency_hz", "high_freq_ratio_35_80"])
...
def main():
    ...
    parser.add_argument("--cut1", type=float, default=30.0)
    parser.add_argument("--cut2", type=float, default=35.0)
    ...
    for label, (freqs, amp, _, _) in spectra.items():
        ...
        print(f"{label}: peak={peak:.2f} Hz, 35-80Hz ratio={rows[-1][2]:.4f}")
