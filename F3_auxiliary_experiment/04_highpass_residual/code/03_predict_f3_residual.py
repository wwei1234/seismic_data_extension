...
def main():
    ...
    parser.add_argument("--model", default=str(CHECKPOINT_DIR / "best_highpass_residual_model.pth"))
    ...
    parser.add_argument("--output-prefix", default="highpass_residual_f3_full")
    args = parser.parse_args()

    ensure_dirs()
    checkpoint = torch.load(args.model, map_location="cpu")
    base_c = checkpoint.get("base_c", 32)
    residual_band = tuple(checkpoint.get("residual_band", RESIDUAL_BAND))
    ...
    for out_idx, il_idx in enumerate(range(i0, i1)):
        ...
        residual_norm = bandpass_section(residual_norm, DT, residual_band)
