...
from common import CHECKPOINT_DIR, DATA_DIR, DT, LOG_DIR, RESIDUAL_BAND, ensure_dirs
...
class TorchTrapezoidBandpass(nn.Module):
    def __init__(self, dt, band):
        super().__init__()
        self.dt = float(dt)
        self.band = tuple(float(v) for v in band)

    def forward(self, x):
        nt = x.shape[-2]
        freqs = torch.fft.rfftfreq(nt, d=self.dt, device=x.device)
        f1, f2, f3, f4 = self.band
        ...
        filt = torch.clamp(filt, 0.0, 1.0).view(1, 1, -1, 1)
        spec = torch.fft.rfft(x, dim=-2)
        return torch.fft.irfft(spec * filt, n=nt, dim=-2)
...
class ResidualWideLoss(nn.Module):
    def __init__(self):
        ...
        self.residual_filter = TorchTrapezoidBandpass(DT, RESIDUAL_BAND)

    def forward(self, x, raw_residual_pred, wide_target, residual_target):
        residual_pred = self.residual_filter(raw_residual_pred)
...
def plot_history(history):
    ...
    keys = ["total", "mae", "freq", "phase", "residual", "dc"]
    epochs = np.asarray(
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.legend()
    fig.tight_layout()
    fig.savefig(LOG_DIR / "highpass_residual_training_curves.png", dpi=300)
    plt.close(fig)
...
def main():
    ...
    history = {f"{split}_{key}": [] for split in ("train", "val")
               for key in ("total", "mae", "freq", "phase", "residual", "dc")}
    history["epoch"] = []
    history["lr"] = []
    best_val = float("inf")
    no_improve = 0
    ...
    if args.resume:
        ...
        history_path = LOG_DIR / "highpass_residual_training_history.npy"
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if val["total"] < best_val:
            ...
            }, CHECKPOINT_DIR / "best_highpass_residual_model.pth")
        else:
            no_improve += 1
        ...
        torch.save({
            ...
            "base_c": args.base_c,
            "residual_band": RESIDUAL_BAND,
        }, CHECKPOINT_DIR / "last_highpass_residual_model.pth")

    np.save(LOG_DIR / "highpass_residual_training_history.npy", history)
