"""Generate synthetic training dataset — 10_raw_wavelet_narrow_norm.

Changes from 09:
  1. Use raw estimated wavelet (no band-shaping), convolve → wide label,
     then low-pass filter wide → narrow input.
  2. Normalise using NARROW's 99th percentile (not wide's), shared for both.
  3. Stride = 128 (was 64).
  4. Keep only horizontal flip augmentation (no vertical flip).
  5. Faults: through-going dipping faults at random angles [75,60,45,30] deg,
     throw halved (±14 main, ±8 secondary).
  6. Low-cut off.
"""
...
# ── Structural reflectivity with dipping faults ──────────────────────────────
...
def build_structural_reflectivity(r1d, width, rng):
    ...
    # ── fault setup (applied after section is built) ──
    fault_info = []
    if rng.random() < 0.65:
        fault_x = int(rng.integers(width // 4, width * 3 // 4))
        fault_throw = float(rng.uniform(-14.0, 14.0))
        fault_angle = float(rng.choice([75, 60, 45, 30]))
        fault_info.append({"x": fault_x, "throw_samples": fault_throw, "angle_deg": fault_angle})
    if rng.random() < 0.25:
        fault_x = int(rng.integers(width // 5, width * 4 // 5))
        fault_throw = float(rng.uniform(-8.0, 8.0))
        fault_angle = float(rng.choice([75, 60, 45, 30]))
        fault_info.append({"x": fault_x, "throw_samples": fault_throw, "angle_deg": fault_angle})

    # Build section without faults
...
def _apply_dipping_fault(section, fx, throw, angle_deg):
    """Apply a through-going dipping fault (top to bottom).

    The fault plane cuts through the entire section at *angle_deg* from
    horizontal.  Samples on the down-thrown side are shifted vertically
    by *throw* samples (positive = down to the right).
    """
...
def cut_patches(input_section, label_section, patch_size, stride):
    ...
    for t0 in t_starts:
        for x0 in x_starts:
            inputs_s.append(input_section[t0:t0 + patch_size, x0:x0 + patch_size])
            labels_s.append(label_section[t0:t0 + patch_size, x0:x0 + patch_size])
            locations.append((t0, x0))
    return np.stack(inputs_s), np.stack(labels_s), locations


def split_train_val(inputs_s, labels_s, metadata, rng, train_ratio=0.8):
    order = rng.permutation(inputs_s.shape[0])
    n_train = int(round(inputs_s.shape[0] * train_ratio))
    train_idx = order[:n_train]
    val_idx = order[n_train:]
    return {
        "train_inputs": inputs_s[train_idx],
        "train_labels": labels_s[train_idx],
        "val_inputs": inputs_s[val_idx],
        "val_labels": labels_s[val_idx],
...
def main():
    ...
    # ── Load RAW estimated wavelets (no band shaping) ──
    ...
    for well_name, reflectivity in reflectivities.items():
        ...
        for section_idx in range(args.sections_per_wavelet):
            ...
            # Convolve with RAW wavelet → wide label
            clean_wide = convolve_reflectivity(base_reflectivity, wavelet)
            # Low-pass filter wide → narrow input
            clean_narrow = zero_phase_filter_section(clean_wide, DT, NARROW_BAND)

            # Q filter on convolved traces
            ...
            # Low-frequency removal (off by default)
            if args.lowcut > 0:
            ...
            # ── Normalisation: use NARROW's percentile ──
    ...
    split = split_train_val(inputs_arr, labels_arr, patch_metadata, rng, train_ratio=0.8)
    ...
    axes[0].set_title(f"{first_key} narrow input (shared norm, stride={args.stride})")
    ...
    # Single trace overlay
    mid_trace = args.section_width // 2
    time_axis = np.arange(section_inputs[first_key].shape[0]) * DT
    fig, ax = plt.subplots(figsize=(12, 4))
    ax.plot(time_axis, section_inputs[first_key][:, mid_trace], "b-", lw=1.2, label="Narrow input")
    ax.plot(time_axis, section_labels[first_key][:, mid_trace], "r--", lw=1.2, label="Wide label")
    ax.invert_yaxis()
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Normalized amplitude")
    ax.set_title(f"{first_key} — single trace overlay")
    ...
    print(f"all patches: {inputs_arr.shape}")
