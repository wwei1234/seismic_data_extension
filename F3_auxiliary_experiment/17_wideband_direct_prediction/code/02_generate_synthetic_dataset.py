"""Generate geometry-realistic direct-wideband samples for experiment 17."""
...
def plot_synthetic_section_examples(
    section_inputs, section_clean_inputs, section_labels, section_meta
):
    ...
    for n_wells in (2, 3, 4):
        ...
        clean_input = section_clean_inputs[sid]
        wide = section_labels[sid]
        diagnostic_residual = wide - clean_input
        panels = [
            (narrow, "Low-pass input"),
            (wide, "Wide-band training label"),
            (diagnostic_residual, "Wide - clean low-pass"),
        ]
        shared_clip = max(
...
def main():
    ...
    for combo_idx, (subset, n_wells) in enumerate(all_combos):
        for wavelet_idx, wavelet_item in enumerate(wavelet_bank):
            for noise_idx, noise_level in enumerate(noise_levels):
                ...
                input_norm, label_norm, shared_scale = prepare_training_pair(
                    clean_narrow, clean_wide, noise_level, rng
                )
    ...
    plot_synthetic_section_examples(
        section_inputs, section_clean_inputs, section_labels, section_meta
    )
    ...
    axes[1].set_title(f"{first_key} wide-band label")
    for ax in axes:
        ax.set_xlabel("Trace")
        ax.set_ylabel("Time sample")
    fig.tight_layout()
    ...
    freqs_l, amp_l = average_amplitude_spectrum(section_labels[first_key], DT)
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(freqs_n, amp_n, "b-", lw=2, label="Narrow")
    ax.plot(freqs_w, amp_w, "r-", lw=2, label="Wide")
    ax.plot(freqs_l, amp_l, "g-", lw=2, label="Training wide label")
    ax.set_xlim(0, 120)
    ax.set_ylim(0, 1.05)
    ax.set_xlabel("Hz")
