...
def patch_starts(n, patch_size=PATCH_SIZE, stride=PATCH_STRIDE):
    ...
    if not starts or starts[-1] != n - patch_size:
        starts.append(n - patch_size)
    return starts


def cut_patches(input_section, label_section, patch_size=PATCH_SIZE, stride=PATCH_STRIDE):
    inputs, labels, locs = [], [], []
    for t0 in patch_starts(input_section.shape[0], patch_size, stride):
        for x0 in patch_starts(input_section.shape[1], patch_size, stride):
            ...
            locs.append((int(t0), int(x0)))
    return np.stack(inputs).astype(np.float32), np.stack(labels).astype(np.float32), locs
...
def main():
    ...
    for n_wells in (2, 3, 4):
        for subset in combinations(well_names, n_wells):
            all_combos.append((tuple(subset), n_wells))

    all_inputs, all_labels, patch_meta = [], [], []
    ...
    for combo_idx, (subset, n_wells) in enumerate(all_combos):
        for wavelet_idx, wavelet in enumerate(wavelet_bank):
            for noise_level in noise_levels:
                ...
                clean_wide = convolve_reflectivity(styled_refl, wavelet["wavelet"])
                clean_wide, texture_info = add_f3_statistical_texture(
                    clean_wide, f3_low, f3_stats["median_metrics"], rng
                )
                clean_wide, horizon_step_info = add_seismic_horizon_step_offsets(clean_wide, rng, min_sample=300)
                ...
                clean_input_norm = (clean_narrow / shared_scale).astype(np.float32)
                wide_norm = (clean_wide / shared_scale).astype(np.float32)
                residual_norm = (wide_norm - clean_input_norm).astype(np.float32)
                label_norm = wide_norm.astype(np.float32)
                input_norm = clean_input_norm.copy()
                if noise_level > 0:
                    noise = rng.normal(0.0, noise_level, size=input_norm.shape).astype(np.float32)
                    input_norm = (input_norm + noise).astype(np.float32)
                ...
                all_inputs.extend(px)
                all_labels.extend(py)
                for t0, x0 in locs:
                    patch_meta.append({
                        "section_id": sid,
                        "wells": list(subset),
                        ...
                        "normalization_scale": float(shared_scale),
                    })

                section_inputs[sid] = input_norm
                section_clean_inputs[sid] = clean_input_norm
                section_labels[sid] = label_norm
                section_wide_labels[sid] = wide_norm
                metrics_dict = {
                    "synthetic_lowpass": section_metrics(clean_input_norm),
                    "synthetic_wide": section_metrics(wide_norm),
                    "residual_label": section_metrics(residual_norm),
                    "f3_lowpass_style_reference": section_metrics(
                        np.clip(f3_low / (np.percentile(np.abs(f3_low), 99) + 1e-8), -1.0, 1.0)
                    ),
                }
                section_meta.append({
                    "section_id": sid,
                    ...
                    "n_wells": int(n_wells),
                    "combo_idx": int(combo_idx),
                    "wavelet_idx": int(wavelet_idx),
                    "wavelet_name": wavelet["name"],
                    "wavelet_band": wavelet["band"],
                    "noise_level": float(noise_level),
                    "num_patches": int(len(locs)),
                    "patch_index_start": int(start),
                    "patch_index_end": int(start + len(locs)),
                    "ordered_wells": info["ordered_wells"],
                    "well_positions": info["positions"],
                ...
                summary[sid] = section_meta[-1]
                target = f3_stats["median_metrics"]
                for panel_name, panel_metrics in metrics_dict.items():
                    row = {"section_id": sid, "panel": panel_name}
                    row.update(panel_metrics)
                    row["target_xlag1_corr"] = target["xlag1_corr"]
                    row["target_grad_x_over_t"] = target["grad_x_over_t"]
                    metric_rows.append(row)

                if section_index in (0, 18, 60):
                    panels = {
                        "synthetic_lowpass": clean_input_norm,
                        "synthetic_wide": wide_norm,
                        "residual_label": residual_norm,
                        "f3_lowpass_style_reference": np.clip(
                            f3_low / (np.percentile(np.abs(f3_low), 99) + 1e-8), -1.0, 1.0
                        ),
                    }
                    title = f"F3-style training sample | {sid} | wells={','.join(subset)} | {wavelet['name']}"
                    plot_section(FIGURE_DIR / f"{sid}_f3_style_training_section.png", title, panels)
                    plot_deep_zoom(FIGURE_DIR / f"{sid}_f3_style_training_deep_zoom.png", title, panels)
                    plot_spectra(FIGURE_DIR / f"{sid}_f3_style_training_spectra.png", title, panels)
                print(f"generated {sid}: patches={len(locs)}")
                section_index += 1
    ...
    np.save(DATA_DIR / "synthetic_patch_metadata.npy", {"train": split["train_metadata"], "val": split["val_metadata"]})
    np.save(DATA_DIR / "synthetic_metadata.npy", {
        "dt": DT,
        "patch_size": int(args.patch_size),
        "stride": int(args.stride),
        "section_shape": [int(reflectivity.shape[0]), int(reflectivity.shape[1])],
        "num_sections": int(len(section_meta)),
    ...
    with (FIGURE_DIR / "f3_style_metric_comparison.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    label_clip = max(float(np.percentile(np.abs(split["train_labels"][0]), 99)), 1e-8)
    axes[1].imshow(split["train_labels"][0], cmap="seismic", aspect="auto", vmin=-label_clip, vmax=label_clip)
    axes[1].set_title("Train patch wide label")
    for ax in axes:
        ax.set_xlabel("Trace")
        ax.set_ylabel("Time")
    fig.tight_layout()
    ...
    print(f"Profiles: {section_index}, patches: {inputs_arr.shape}")
