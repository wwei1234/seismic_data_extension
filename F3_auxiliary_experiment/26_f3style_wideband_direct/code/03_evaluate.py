...
from config import DATA_DIR, DT, HIGH_BAND
...
def high_frequency_ratio(frequencies, amplitude):
    high = (frequencies >= HIGH_BAND[0]) & (frequencies <= HIGH_BAND[1])
    total = (frequencies >= 1.0) & (frequencies <= HIGH_BAND[1])
...
def evaluate_cube(cube, target):
    ...
    result.update({
            "high_freq_ratio_35_90": high_frequency_ratio(frequencies, amplitude),
...
def format_metrics(title, result):
    ...
    for key in (
        ...
        "high_freq_ratio_35_90", "spectrum_l1_vs_reference",
    ):
...
def plot_spectra(output, spectra):
    ...
    ax.axvspan(HIGH_BAND[0], HIGH_BAND[1], color="tab:red", alpha=0.10, label="35-90 Hz")
    ax.set_xlim(0, 120)
...
def main():
    ...
    figure_dir = DATA_DIR.parent / "figures" / "prediction_evaluation"
