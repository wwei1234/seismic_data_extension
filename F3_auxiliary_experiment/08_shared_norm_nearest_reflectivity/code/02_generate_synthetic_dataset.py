...
def build_structural_reflectivity(r1d, width, rng):
    ...
    for ix, shift in enumerate(shifts):
        ...
        sample_idx = np.rint(sample_pos).astype(np.int32)
        valid = (sample_idx >= 0) & (sample_idx < n_time)
        col = np.zeros(n_time, dtype=np.float32)
        col[valid] = r1d[sample_idx[valid]]
        section[:, ix] = col
...
def main():
    ...
    for well_name, reflectivity in reflectivities.items():
        ...
        for section_idx in range(args.sections_per_wavelet):
            ...
            shared_scale = float(np.percentile(np.abs(clean_wide), 99.0))
