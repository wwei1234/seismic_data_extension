...
def main():
    ...
    for il_idx in range(n_inline):
        ...
        # Use consistent normalisation: same as training (percentile-based)
        ref_scale = float(np.percentile(np.abs(narrow_raw), 99))
        ref_scale = max(ref_scale, 1e-8)
        narrow_norm = np.clip(narrow_raw / ref_scale, -1.0, 1.0).astype(np.float32)
        mn, mx = -ref_scale, ref_scale
