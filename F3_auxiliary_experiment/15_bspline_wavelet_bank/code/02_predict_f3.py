...
Pipeline:
    ...
    5. Apply targeted spectral gain correction to 35-75 Hz
...
def main():
    ...
    parser.add_argument("--hp-cut1", type=float, default=30.0)
    parser.add_argument("--hp-cut2", type=float, default=35.0)
    ...
    for il_idx in range(n_inline):
        ...
        pred_arr[il_idx] = wide_direct.astype(np.float32)
        direct_arr[il_idx] = wide_direct.astype(np.float32)
        hp_pred_arr[il_idx] = wide_highpass.astype(np.float32)
        residual_arr[il_idx] = residual_pred.astype(np.float32)
        residual_hp_arr[il_idx] = residual_hp.astype(np.float32)
