...
def supervised_traces(signal, supervision_mask):
    selected = []
    for index in range(signal.shape[0]):
        columns = supervision_mask[index, 0].amax(dim=0) > 0
        selected.append(signal[index:index + 1, :, :, columns])
    widths = {item.shape[-1] for item in selected}
    if len(widths) != 1:
        raise ValueError("Every sample must supervise the same trace count.")
...
class DomainAwarePhaseLoss(nn.Module):
    ...
    def forward(
        ...
        supervised_input = input_data
        supervised_wide = target_wide
        if supervision_mask is not None:
            projected_prediction = supervised_traces(
                projected_prediction, supervision_mask
            )
            projected_target = supervised_traces(
                projected_target, supervision_mask
            )
            supervised_input = supervised_traces(
                input_data, supervision_mask
            )
            supervised_wide = supervised_traces(
                target_wide, supervision_mask
            )
        ...
        reconstructed = supervised_input + projected_prediction
        wide = F.smooth_l1_loss(
            reconstructed, supervised_wide, beta=0.05
        )
